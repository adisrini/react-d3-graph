...
function _createForceSimulation(width, height) {
    ...
    return (0, _d3Force.forceSimulation)()
        .force('charge', (0, _d3Force.forceManyBody)().strength(_const2.default.FORCE_IDEAL_STRENGTH))
        .force('x', frx)
        .force('y', fry);
}
...
function buildLinkProps(
    ...
    if (highlight) {
        stroke =
            config.link.highlightColor === _const2.default.KEYWORDS.SAME
                ? config.link.color
